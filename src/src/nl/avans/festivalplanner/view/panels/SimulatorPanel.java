...
import nl.avans.festivalplanner.model.simulator.Block;
...
public class SimulatorPanel extends Panel
{
	...
	public class Toolbar extends JPanel implements ActionListener
	{
		...
		private JScrollPane getRemainingTab()
		{
			...
			FestivalHandler.Instance().addRemaining(new Block());
			Panel pane = new Panel()
			{
				@Override
				protected void paintComponent(Graphics g)
			...
			MouseMotionListener
	{
		...
		public void mouseReleased(MouseEvent e)
		{
			...
			if (!FestivalHandler.Instance().getElementsOnTerrain().contains(elementDraggedFromToolbar)&& elementDraggedFromToolbar != null && e.getPoint().getX() < 0)
			{
				...
				{
					if(elementDraggedFromToolbar instanceof Block) // resizing from 100x100 to 100x25
						((Block)elementDraggedFromToolbar).setSize(new Dimension(100, 25));
